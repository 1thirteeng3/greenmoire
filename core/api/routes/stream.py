...
async def chat_websocket(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            event = BaseEvent(
                header=EventHeader(trace_id=trace_id, source_service="api_gateway_ws", event_type="user_prompt_received"),
            ...
            while (asyncio.get_event_loop().time() - start_time) < timeout_seconds:
                ...
                streams = await bus.client.xread({reply_stream: last_id}, count=10, block=500)
                ...
                if streams:
                    for stream_name, messages in streams:
                        for msg_id, msg_data in messages:
                            ...
                            try:
                                reply_event = BaseEvent.model_validate_json(msg_data["payload"])
                            except Exception as e:
                                logger.error(f"Erro ao parsear resposta: {e}")
                                continue
                            
                            # Se for a resposta final
                            if reply_event.header.event_type == "cognitive_response_delivered":
                                ...
                                await websocket.send_json({
                                    ...
                                    "role": "grimoire",
                                    "content": reply_event.payload.get("response", ""),
                                    "metadata": {
                                        "tier_used": reply_event.payload.get("tier_used"),
                                        "primary_intent": reply_event.payload.get("primary_intent"),
