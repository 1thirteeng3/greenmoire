...
class AsyncRedisEventBus:
    ...
    async def publish(
        self, stream_name: str, event: BaseEvent, maxlen: int = 100000
    ) -> str:
        """Publica evento de forma assíncrona com limite de tamanho de stream."""
        event_data = {"payload": event.model_dump_json()}
        message_id = await self.client.xadd(
            name=stream_name, fields=event_data, maxlen=maxlen, approximate=True
        )
        return message_id
    ...
    async def reclaim_pending_messages(
        ...
        """
        Recupera mensagens pendentes de workers mortos.
        Tenta XAUTOCLAIM (Redis ≥ 6.2) e cai de volta para XPENDING + XCLAIM
        em versões mais antigas, evitando o erro 'unknown command XAUTOCLAIM'.
        """
        # Tenta o caminho moderno (Redis ≥ 6.2)
        try:
            ...
            if "unknown command" not in str(e).lower():
                # Outro erro de Redis — loga mas não bloqueia
                logger.error(
                    f"Erro ao reivindicar mensagens pendentes no stream {stream_name}: {e}"
                )
                return []
            # XAUTOCLAIM não existe nesta versão do Redis: usa XPENDING + XCLAIM
            return await self._reclaim_via_xpending(
                stream_name, group_name, consumer_name, min_idle_time_ms, batch_size
            )
        except Exception as e:
            logger.error(
                f"Erro ao reivindicar mensagens pendentes no stream {stream_name}: {e}"
    ...
    async def _reclaim_via_xpending(
        self,
        stream_name: str,
        group_name: str,
        consumer_name: str,
        ...
        """
        Fallback para Redis < 6.2: XPENDING lista mensagens idle,
        XCLAIM transfere a propriedade para este consumer.
        """
        try:
            # XPENDING retorna: [msg_id, consumer, elapsed_ms, delivery_count]
            ...
            # Filtra apenas mensagens idle há mais de min_idle_time_ms
            ids_to_claim = [
                entry["message_id"]
            ...
            if claimed:
                return self._parse_messages([[stream_name, claimed]])
            return []

        except Exception as e:
            logger.error(f"Erro no fallback XPENDING+XCLAIM para {stream_name}: {e}")
            return []

    def _parse_messages(self, raw_messages: list) -> List[Tuple[str, BaseEvent]]:
        parsed_events: List[Tuple[str, BaseEvent]] = []
        if not raw_messages:
        ...
        for stream, msgs in raw_messages:
            for message_id, msg_data in msgs:
                try:
                    ...
                    logger.critical(
                        f"Falha fatal de serialização na mensagem {message_id}: {e}"
                    )
        return parsed_events
