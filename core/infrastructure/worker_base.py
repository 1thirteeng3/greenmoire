...
class BaseEventWorker:
    ...
    async def start(
        self, handler_func: Callable[[BaseEvent, AsyncSession], Awaitable[None]]
    ):
        await self.bus.setup_consumer_group(self.stream_name, self.group_name)
        self._running = True
        logger.info(
            f"Worker {self.consumer_name} iniciado no stream {self.stream_name}."
        )
    ...
    async def _process_batch(self, handler_func: Callable, is_recovery: bool):
        if is_recovery:
            messages = await self.bus.reclaim_pending_messages(
            ...
            async with self.session_factory() as session:
                try:
                    ...
                    await self._send_to_dlq(
                        event, message_id, fatal_error, "FATAL_DB_OR_CONTRACT"
                    )
                    await self.bus.acknowledge(
                        self.stream_name, self.group_name, message_id
                    )
                    ...
                    await session.rollback()
                    logger.warning(
                        f"Erro transiente no evento {message_id}: {e}. Retentativa gerenciada pelo PEL."
                    )
