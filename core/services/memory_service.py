...
class MemoryServiceWorker(BaseEventWorker):
    def __init__(
        ...
        super().__init__(
            bus,
            session_factory,
            stream_name="stream:memory",
            group_name="memory_service_group",
            consumer_name="memory_worker_1",
        )
    ...
    async def handle_event(self, event: BaseEvent, session: AsyncSession):
        ...
        if event_type == "semantic_memory_store":
            ...
            await repository.save_semantic_memory(
                ...
                human_verified=payload.get("human_verified", False),
            )
            ...
            error_embedding = await self.embedding_provider.generate_embedding(
            ...
            results = await repository.search_semantic_memory(
                embedding, payload.get("domain"), payload.get("limit", 5)
            )
            ...
            reply_event = BaseEvent(
                header=EventHeader(
                    ...
                    event_type="semantic_memory_result",
                ),
                payload={
                    "results": [
                        {"id": str(m.id), "content": m.content, "score": s}
                        for m, s in results
                    ],
                    "original_query": payload["query"],
                },
            )
            reply_to_stream = (event.metadata or {}).get("reply_to_stream")
            if reply_to_stream:
